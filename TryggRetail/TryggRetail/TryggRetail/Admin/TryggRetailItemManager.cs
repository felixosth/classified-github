...
namespace TryggRetail.Admin
{
    ...
    /// </summary>
    public class TryggRetailItemManager : ItemManager
    {
        private TryggRetailUserControl _userControl;
        private Guid _kind;

        #region Constructors

        public TryggRetailItemManager(Guid kind) : base()
        {
            _kind = kind;
        }
        ...
        {
            _userControl = new TryggRetailUserControl();
        ...
        {
            return new TryggRetailAddUserControl();
        }
        ...
        public override bool ValidateAndSaveUserControl()
        {
            if (CurrentItem != null)
            {
                ...
                Configuration.Instance.SaveItemConfiguration(TryggRetailDefinition.TryggRetailPluginId, CurrentItem);
            }
            return true;
        }
        ...
        {
            CurrentItem = new Item(suggestedFQID, ((TryggRetailAddUserControl)addUserControl).ItemName);
            var defaultValues = new PopupConfig()
            {
                AckTime = 3,
                AlarmName = "AlarmName",
                AlarmPlaybackOffsetAfter = 3,
                AlarmPlaybackOffsetBefore = 5,
                AlarmSignal = "",
                LoopAlarmSignal = false,
                LoopByDefault = true,
                WindowSize = new WindowSize() { Width = 1280, Height = 720 },
                WindowPosition = new WindowPos() { X = 50, Y = 30 },
                IsFullscreen = true,
                CustomActions = new List<Actions.CustomAction>(),
                StopSoundOnExit = false,
                ButtonBlink = false
            };

            CurrentItem.Properties["configuration"] = defaultValues.Serialize();
        ...
        public override void DeleteItem(Item item)
        {
            if (item != null)
            {
                Configuration.Instance.DeleteItemConfiguration(TryggRetailDefinition.TryggRetailPluginId, item);
            }
        ...
        public override List<Item> GetItems()
        {
            ...
            List<Item> items = Configuration.Instance.GetItemConfigurations(TryggRetailDefinition.TryggRetailPluginId, null, _kind);
            return items;
        }
        ...
        {
            List<Item> items = Configuration.Instance.GetItemConfigurations(TryggRetailDefinition.TryggRetailPluginId, parentItem, _kind);
            return items;
        }
        ...
        {
            Item item = Configuration.Instance.GetItemConfiguration(TryggRetailDefinition.TryggRetailPluginId, _kind, fqid.ObjectId);
            return item;
        }
