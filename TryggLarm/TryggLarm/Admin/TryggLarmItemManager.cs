...
namespace TryggLarm.Admin
{
    ...
    /// </summary>
    public class TryggLarmItemManager : ItemManager
    {
        //private TryggLarmUserControl _userControl;
        private NodeEditorManager _userControl;
        private Guid _kind;

        #region Constructors

        public TryggLarmItemManager(Guid kind) : base()
        {
            _kind = kind;
        }
        ...
        {
            _userControl = new NodeEditorManager();
        ...
        {
            return new TryggLarmAddUserControl();
        }
        ...
        public override bool ValidateAndSaveUserControl()
        {
            if (CurrentItem != null)
            {
                ...
                Configuration.Instance.SaveItemConfiguration(TryggLarmDefinition.TryggLarmPluginId, CurrentItem);
            }
            return true;
        }
        ...
        {
            var ctrl = ((TryggLarmAddUserControl)addUserControl);
            CurrentItem = new Item(suggestedFQID, ctrl.ItemName);

            using (MemoryStream ms = new MemoryStream())
            {
                BinaryFormatter bf = new BinaryFormatter();
                bf.Serialize(ms, new RootNode(ctrl.ItemName));
                ms.Position = 0;
                byte[] buffer = new byte[(int)ms.Length];
                ms.Read(buffer, 0, buffer.Length);
                CurrentItem.Properties["treeNodes"] = Convert.ToBase64String(buffer);
            }
        ...
        public override void DeleteItem(Item item)
        {
            if (item != null)
            {
                Configuration.Instance.DeleteItemConfiguration(TryggLarmDefinition.TryggLarmPluginId, item);
            }
        ...
        public override List<Item> GetItems()
        {
            ...
            List<Item> items = Configuration.Instance.GetItemConfigurations(TryggLarmDefinition.TryggLarmPluginId, null, _kind);
            return items;
        }
        ...
        {
            List<Item> items = Configuration.Instance.GetItemConfigurations(TryggLarmDefinition.TryggLarmPluginId, parentItem, _kind);
            return items;
        }
        ...
        {
            Item item = Configuration.Instance.GetItemConfiguration(TryggLarmDefinition.TryggLarmPluginId, _kind, fqid.ObjectId);
            return item;
        }
